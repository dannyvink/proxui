...
class Hid
{
    ...
    public function clone(Proxmark3 $scanner) {
        $command = $scanner->executeCommand("lf hid clone " . $this->identifier);

        if ($command["success"] && preg_match('/cloning tag with id/im', $command['result'])) {
            return true;
        }
    ...
    public static function tagFromResult($result) {
        $matches = [];
        preg_match('/hid prox tag id: (\w*) /i', $result, $matches);
        if ($matches) {
            array_shift($matches);
